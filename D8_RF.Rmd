---
title: "Decision tree, Random forest & Gradient boosting"
author: "Hong, Yoon-Ho"
date: "2022-09-10"
output: 
  html_document:
...
그러나, 설명변수 공간을 J개로 분할하는 모든 가능한 경우를 고려하는 것은 계산상 실현이 불가능하므로, 
top-down 방식의 반복적인 이분 분할 전략을 이용합니다(**recursive binary splitting**)
...
$$E = 1 - \max(\hat{p}_{mk})$$
$\hat{p}_{mk}$: m번째 설명변수 공간 내 k class에 속하는  관측치들의 비율      
...
회귀 에서는 평균값을(average for regression), 분류에서는 다수 모델의 예측치에 따라 결정됩니다(majority rule for classification)    
...
**Variable importance** 
주어진 설명변수에 대한 분할로 인한 검정 오차(RSS or Gini index/Cross entropy)의 감소분을 모든(B개) 트리에 대해 평균합니다. 이 값이 크면 해당 설명변수가 중요하다고 할 수 있습니다.   
...
random forest 모델의 두가지 tuning parameter는 ntree와 mtry입니다.   
...
```{r}
set.seed(9)
gbm_model = train(medv~., data = train_boston, trControl = trctrl, method = "gbm", verbose = F) # gradient boosting machine    
```
...
```{r} 
gbm_model = train(medv~., data = train_boston, trControl = trctrl, method = "gbm", tuneGrid = gbmGrid,  verbose = F) # gradient boosting machine    
```
